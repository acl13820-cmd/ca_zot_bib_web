...
def check_requirements ():
    try:
        v = float("%d.%02d%02d" % tuple(map(int, getattr(zotero, '__version__', 'unknown')
.split(r'.'))))
        if v < 1.0103:
...
class Settings:
    ...
    def make_arg_parser():
        ...
        if __name__ == '__main__':
            v = "ZBW %s - Pyzotero %s - Python %s"%(__version__, getattr(zotero, '__version__', 'unknown'), sys.version)
...
class DBInstance:
    ...
    def retrieve_data_cached(self, collection_id, exclude=None):
                ...
                if date > datetime.now() - timedelta(days=14):  # cache expires after 14 days
                    if self.zotLastMod == lm and bs == bib_style:
                        if len(items) == 0 or (hasattr(items[0], '__version__') and
                                                       ...
                                                       zv == getattr(zotero, '__version__', 'unknown')
):
                            return items
                        else:
                            # log("version diff")
        ...
        if not no_cache:
            ...
            pickle.dump((datetime.now(), self.zotLastMod, bib_style, getattr(zotero, '__version__', 'unknown')
, a),
                        open(cache_name, 'wb'))
